  ...
  return (
    <section className="pointer-events-none sm:pointer-events-auto py-20 sm:py-[12.25rem]">
      <FadeUp>
        ...
        >
          {words.map((word, i) => (
            <SwiperSlide key={i} className="relative py-3 !w-auto !flex !flex-row-reverse items-center gap-3 sm:gap-6">
              <div
                className={twm(
              ...
              {word.dot && <div className="w-3 sm:w-5 h-3 sm:h-5 bg-[#D9D9D9] rounded-full -mb-4 sm:-mb-7" />}
            </SwiperSlide>
          ))}
          {words.map((word, i) => (
            <SwiperSlide key={i} className="relative py-3 !w-auto !flex items-center gap-3 sm:gap-6">
              <div
                className={twm(
                  'text-[3.75rem] sm:text-9xl font-medium leading-[1.1] -tracking-[0.15rem] sm:-tracking-[0.32rem]',
                  word.color
