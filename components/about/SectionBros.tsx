          ...
          {aboutPage.bros.map((bro: Bro, index: number) => (
            ...
            >
              ...
              <div className="text-[2.5rem] sm:text-[3.75rem] leading-[1.5] font-medium -tracking-[0.1rem] sm:-tracking-[0.15rem] mt-5 sm:mt-8 mb-2 sm:mb-0">
                <AnimatedText>{bro.name}</AnimatedText>
              </div>
              <div className="grid grid-cols-2 mb-5 sm:mb-8 max-w-[25.75rem]">
                {bro.positions.map((position: string) => (
                  <div
                    key={position}
                    className="w-[20rem] text-lg sm:text-2xl font-normal sm:font-medium leading-[1.5] leading-0 sm:-tracking-[0.03rem] opacity-50"
                  >
                    <AnimatedText>{position}</AnimatedText>
                  </div>
                ))}
              </div>
              <div className="text-lg sm:text-2xl leading-[1.5] tracking-0 sm:-tracking-[0.03rem]">
                <AnimatedPortableText value={bro.bio} />
              </div>
            </div>
          ))}
        </div>
